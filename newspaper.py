...
def downloadPDF():
	...
	dt = datetime.now().replace(tzinfo=pytz.utc).astimezone(pytz.timezone("US/Central")).date()
	hya = dt.replace(year=dt.year - 100, day=dt.day)
	hya_pretty = hya.strftime('%B %d, %Y')

	papers_json_url = 'https://chroniclingamerica.loc.gov/frontpages/' + str(hya) + '.json'
